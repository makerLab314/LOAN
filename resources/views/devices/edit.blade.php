...
<div class="container mx-auto">
    ...
    <form action="{{ route('devices.update', $device) }}" method="POST" enctype="multipart/form-data" class="w-full">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="total_quantity" id="total_quantity" min="1" value="{{ old('total_quantity', $device->total_quantity ?? 1) }}"
            ...
            <p class="text-xs text-gray-500 mt-1">
                Für Geräte mit mehreren Einheiten (z.B. 10 Kabel). 
                @if(isset($device) && $device->loaned_quantity > 0)
                    <span class="text-yellow-600">Aktuell verliehen: {{ $device->loaned_quantity }}</span>
                @endif
            </p>
